...
class ScannetDetectionDataset(Dataset):
    def __init__(
        ...
        self.data_path = DATASET_ROOT + "/scannet_%s" % (split_set)
        ...
        if split_set in ["train"]:
            ...
            self.scan2scene = pk.load(open("scannet_train_scene_label.pkl", "rb"))
            ...
            self.scan2scene = pk.load(open("scannet_val_scene_label.pkl", "rb"))
    ...
    def __getitem__(self, idx):
        while True:
            ...
            if not self.args.train_global:
                break
        
        point_cloud = np.load(scan_path + "_pc.npy").astype(np.float32)       
        bboxes = np.load(scan_path + "_bbox.npy")       
        ...
        ret_dict["scene"] = np.array(self.scene2sceneclass[self.scan2scene[scan_name]]).astype(np.int64)    
        return ret_dict
